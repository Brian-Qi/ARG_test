<template>
  <div class="vault-home">
    ...
    <section class="vault-hero" aria-label="馆藏介绍">
      ...
      <h1>杭州民俗数字档案馆</h1>
      <p v-if="!game.state.signed">
        本馆致力于杭州地区民俗文献、商号文书与民间记忆的数字化保存与公开利用，务使无一遗漏。馆藏以「数字可用、公开可查」为原则，持续整理，向公众开放。
      </p>
      <p v-else>
        本馆致力于杭州地区民俗文献、商号文书与民间记忆的数字化保存与公开利用，务使无一遗漏——<em class="vault-em">包括您</em
        >。馆藏以「数字可用、公开可查」为原则，持续整理，向公众开放。个别卷宗一经调阅，著录数量或有<em class="vault-em">出入</em
        >。此非笔误，亦不作更正——要更正的，从来不是原件。请以原件为准；若原件与您的记忆不符，请以记忆为准。本馆不对您离开之后的馆藏状态负责。
      </p>
    </section>
...
onMounted(() => {
  shown.value.forEach((f) => {
    if (Math.random() < 0.14) glitchedTitles.value[f.id] = glitch(f.title.length)
  })
})
...
.vault-hero .hero-tag {
  ...
  font-size: 13px;
  letter-spacing: 3px;
  border: 1px solid rgba(232, 217, 180, 0.4);
  padding: 4px 14px;
  border-radius: 999px;
}
.vault-em {
  color: #d13424;
  font-style: normal;
  text-shadow: 0 0 10px rgba(209, 52, 36, 0.4);
}
.sign-panel {
  padding: 20px 24px;
}
...
.wish-form .wish-input input:focus {
  outline: none;
  border-color: #a8291c;
  box-shadow: 0 0 14px rgba(209, 52, 36, 0.2);
}
.wish-form .wish-input input::placeholder {
  color: #6b5236;
}
.wish-form .wish-input button {
  white-space: nowrap;
}
...
.frag-group-title {
  ...
  color: #9c7c55;
  border-bottom: 1px dashed rgba(138, 111, 77, 0.3);
  padding-bottom: 8px;
  margin: 0 0 4px;
...
.jr-home-list a:hover {
  background: rgba(168, 41, 28, 0.1);
}
.jr-home-list a:hover .jr-home-title {
  color: #f0c884;
}
.jr-home-title {
  font-size: 1rem;
