...
const related = computed(() => {
  const frags = FRAGMENTS
    .filter(f => (f.entities || []).includes(route.params.id) && (!f.hidden || game.hasRead(f.id)))
    .map(f => ({ kind: 'frag', id: f.id, title: f.title, tag: f.tag, to: '/f/' + f.id }))
  const arts = JOURNAL
    .filter(a => (a.entities || []).includes(route.params.id))
    .map(a => ({ kind: 'article', id: a.id, title: a.title, tag: '研究辑录 · ' + a.author, to: '/journal/' + a.id }))
  return [...frags, ...arts]
})
</script>
