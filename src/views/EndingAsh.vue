<template>
  <section class="ending-page ending-grey">
    <figure class="ep-hero">
      <img :src="art" alt="" width="1672" height="941" />
...
.ep-hero {
  ...
  overflow: hidden;
  border-radius: 6px;
  border: 1px solid rgba(150, 145, 130, 0.4);
  background: #0d0906;
  box-shadow: 0 0 50px rgba(120, 118, 110, 0.14);
  animation: ep-in 0.9s ease both;
}
.ep-hero img {
  width: 100%;
  height: 100%;
  object-fit: cover;
  display: block;
  filter: grayscale(0.28) sepia(0.14) contrast(1.06) brightness(0.86);
  animation: ep-breathe 7s ease-in-out infinite alternate;
}
/* 压暗 + 底部余烬的暖光 + 暗角 */
.ep-hero::before {
  content: '';
...
@keyframes ember-rise {
  ...
  100% {
    transform: translate3d(var(--dx, 12px), -300px, 0) scale(0.35);
    opacity: 0;
  }
}
...
.ep-cap {
  ...
  bottom: 30px;
  z-index: 3;
  animation: cap-in 0.9s ease 0.28s both;
}
.ep-kicker {
  margin: 0 0 8px;
  font-size: 0.82rem;
...
.ep-seal {
  ...
  font-size: 34px;
  border: 2px solid rgba(150, 145, 130, 0.7);
  color: #cfc7b4;
  background: rgba(20, 19, 17, 0.5);
  transform: rotate(-6deg);
  box-shadow: 0 0 22px rgba(120, 118, 110, 0.3);
...
.ep-copy {
  ...
  margin: 34px auto 0;
  text-align: center;
  animation: ep-copy-in 1s ease 0.25s both;
}
.ep-copy p {
  line-height: 2.2;
  font-size: 1.05rem;
...
.grey-hint:hover {
  ...
  text-shadow: 0 0 14px rgba(224, 178, 92, 0.35);
}
...
@keyframes ep-breathe {
  from {
    filter: grayscale(0.28) sepia(0.14) contrast(1.06) brightness(0.82);
  }
  to {
    filter: grayscale(0.28) sepia(0.14) contrast(1.06) brightness(0.92);
  }
}
@keyframes ep-copy-in {
  from {
    opacity: 0;
