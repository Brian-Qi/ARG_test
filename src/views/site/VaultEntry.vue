...
onMounted(() => {
  ...
  if (!(seenHidden() && familyUnlocked() && shenSearched())) {
    game.takeShortcut()
    router.replace('/')
    return
  }
...
.vault-entry.dark .ve-panel {
  ...
  border-color: #a8291c;
  box-shadow:
    0 0 70px rgba(168, 41, 28, 0.45),
    inset 0 0 90px rgba(168, 41, 28, 0.28);
}
...
.vault-entry.corrupt .ve-kicker {
  color: #d13424;
  border-color: rgba(209, 52, 36, 0.6);
}
.ve-title {
...
.vault-entry.corrupt .ve-title {
  color: #d9c69a;
}
.vault-entry.dark .ve-title {
  color: #d13424;
}
.ve-title.glitch {
  animation: ve-glitch 0.16s steps(2) infinite;
}
@keyframes ve-glitch {
  0% {
    transform: translate(0, 0);
  ...
  50% {
    transform: translate(-2px, 1px);
    text-shadow:
      2px 0 rgba(209, 52, 36, 0.7),
      -2px 0 rgba(80, 200, 200, 0.3);
  }
  100% {
    transform: translate(2px, -1px);
    text-shadow: -2px 0 rgba(209, 52, 36, 0.7);
  }
}
.ve-body {
  color: #6d5f45;
...
.vault-entry.corrupt .ve-body {
  color: #9c7c55;
}
.vault-entry.dark .ve-body {
  color: #b09a72;
}
.ve-bar {
  height: 3px;
...
.ve-bar.blood i {
  background: linear-gradient(90deg, #8c2f24, #d13424);
  box-shadow: 0 0 12px rgba(209, 52, 36, 0.6);
}
.ve-meta {
  display: flex;
...
.ve-flash::before {
  animation: ve-glitch-a 0.5s steps(3, end) infinite;
}
.ve-flash::after {
  animation: ve-glitch-b 0.4s steps(4, end) infinite;
